...
class SpecialitationResponseModel{
  ...
  factory SpecialitationResponseModel.fromMap(Map<String, dynamic> map){
    return SpecialitationResponseModel(map['status'] ?? '', List<SpecialitationModel>.from(map['data']?.map((x) => SpecialitationResponseModel.fromMap(x))),);
  }

  String toJson() => json.encode(toMap());

  factory SpecialitationResponseModel.fromModel(String source) => SpecialitationResponseModel.fromMap(json.decode(source));
}
...
class SpecialitationModel{
  ...
  factory SpecialitationModel.fromMap(Map<String, dynamic> map){
    return SpecialitationModel(id: map['id']?.toInt() ?? 0, name: map['name'] ?? '',);
  }
